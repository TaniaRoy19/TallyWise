...
still-unresolved gateway row from the same merchant, and ask it to either:
  ...
  - state that it cannot responsibly resolve it, and say why.
...
- If no candidate is a plausible match, say so.
- Respond ONLY with JSON, no other text, in exactly this shape:
{"match_settlement_id": "<id or null>", "confidence": <0.0-1.0>, "reason": "<one sentence>"}
"""
...
def _heuristic_fallback(ledger_row, candidates):
    ...
    Mimics the same narrow reasoning contract as the LLM so the pipeline is
    fully runnable out of the box, and is clearly labeled as such in output.
    """
    if not candidates:
